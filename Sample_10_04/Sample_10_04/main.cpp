...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
	...
	yBlurSpriteInitData.m_height = 360;
 	...
 	// 【注目】テクスチャはmainRenderTargetと同じ
 	yBlurSpriteInitData.m_textures[0] = &mainRenderTarget.GetRenderTargetTexture();
	...
	// 初期化情報をもとに横ブラー用にスプライトを初期化する 
 	Sprite yBlurSprite;
 	yBlurSprite.Init(xBlurSpriteInitData);
	...
	// ボケ画像をそのまま貼り付けるためだけなので、通常の2D描画のシェーダーを指定する
	spriteInitData.m_fxFilePath = "Assets/shader/sample2D.fx";

	// 初期化オブジェクトを使って、スプライトを使用していく
