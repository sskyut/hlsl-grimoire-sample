...
float4 PSBlur(PS_BlurInput In) : SV_Target0
{
    ...
    // 基準テクセルにマイナス方向尾に8テクセル、重み付きでサンプリング
    ...
    Color += weight[0].x * sceneTexture.Sample(Sampler, In.tex4.zw);
    Color += weight[0].y * sceneTexture.Sample(Sampler, In.tex5.zw);
    Color += weight[0].z * sceneTexture.Sample(Sampler, In.tex6.zw);
    Color += weight[0].w * sceneTexture.Sample(Sampler, In.tex7.zw);
