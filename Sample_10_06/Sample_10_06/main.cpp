...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
	...
	// gaussianBlur[1]は、gaussianBlur[0]のテクスチャーにガウシアンブラーをかける
	gaussianBlur[1].Init(&gaussianBlur[0].GetBokeTexture());

	// gaussianBlur[2]は、gaussianBlur[1]のテクスチャーにガウシアンブラーをかける
	gaussianBlur[2].Init(&gaussianBlur[1].GetBokeTexture());

	// gaussianBlur[3]は、gaussianBlur[2]のテクスチャーにガウシアンブラーをかける
	...
	//【注目】ボケ画像に合成する必要があるので、2D用のシェーダーではなく
	// 専用シェーダーを指定
	finalSpriteInitData.m_fxFilePath = "Assets/shader/samplePostEffect.fx";
	finalSpriteInitData.m_psEntryPoinFunc = "PSBloomFinal";

	// ただし、加算合成で描画するので、アルファブレンディングモードに加算する
	finalSpriteInitData.m_alphaBlendMode = AlphaBlendMode_Add;

	// カラーバッファーのフォーマットは例によって、32の浮動小数点バッファー
	finalSpriteInitData.m_colorBufferFormat[0] = DXGI_FORMAT_R32G32B32A32_FLOAT;

	// 初期化情報をもとに加算合成用スプライトを初期化する
	Sprite finalSprite;
